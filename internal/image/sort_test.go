...
// threshold, so the slice has to be long! The ranks also have
// have to interleave: given a comparator that reports everything equal,
...
func TestNewestFirstReadsABareYearAsTheStartOfIt(t *testing.T) {
	...
	dates := map[uuid.UUID]*string{year: at("2019"), june: at("2019-06")}
...
func TestRankOutranksDate(t *testing.T) {
	...
	dates := map[uuid.UUID]*string{ranked: at("1999"), newer: at("2024")}
