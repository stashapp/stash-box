...
const getImageDatesForEdit = `-- name: GetImageDatesForEdit :many
WITH edit AS (
...
type GetImageDatesForEditRow struct {
...
func (q *Queries) GetImageDatesForEdit(ctx context.Context, id uuid.UUID) ([]GetImageDatesForEditRow, error) {
	rows, err := q.db.Query(ctx, getImageDatesForEdit, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetImageDatesForEditRow{}
	for rows.Next() {
		var i GetImageDatesForEditRow
		if err := rows.Scan(&i.ImageID, &i.Date); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const getImageTypesForEdit = `-- name: GetImageTypesForEdit :many
WITH edit AS (
  SELECT id, user_id, operation, target_type, data, votes, status, applied, created_at, updated_at, closed_at, bot, update_count FROM edits WHERE edits.id = $1
), final_images AS (
    SELECT fi.image_id FROM edit_final_images fi WHERE fi.edit_id = $1
...
type GetImageTypesForEditRow struct {
	ImageID uuid.UUID `db:"image_id" json:"image_id"`
...
func (q *Queries) GetImageTypesForEdit(ctx context.Context, id uuid.UUID) ([]GetImageTypesForEditRow, error) {
	rows, err := q.db.Query(ctx, getImageTypesForEdit, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetImageTypesForEditRow{}
	for rows.Next() {
		var i GetImageTypesForEditRow
...
SELECT i.id, i.url, i.width, i.height, i.checksum FROM edit_final_images fi
...
func (q *Queries) GetImagesForEdit(ctx context.Context, editID uuid.UUID) ([]Image, error) {
	rows, err := q.db.Query(ctx, getImagesForEdit, editID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
