...
-- name: CreatePerformerImageTypes :copyfrom
INSERT INTO performer_image_types (performer_id, image_id, type_key) VALUES ($1, $2, $3);

-- name: FindImageDatesByPerformerIds :many
SELECT performer_id, image_id, date
FROM performer_images
WHERE performer_id = ANY(sqlc.arg(performer_ids)::UUID[]);

-- name: DeletePerformerImageTypes :exec
DELETE FROM performer_image_types WHERE performer_id = $1;

-- name: FindImageTypesByPerformerIds :many
...
WHERE pit.performer_id = ANY(sqlc.arg(performer_ids)::UUID[])
