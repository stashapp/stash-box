...
func confirmOldEmail(ctx context.Context, tx *db.Queries, user models.User) error {
	...
	return sendConfirmOldEmail(user, *key)
}
...
func confirmNewEmail(ctx context.Context, tx *db.Queries, user models.User, email string) error {
	// generate an activation key and email
	...
	return sendConfirmNewEmail(&user, email, *key)
}
...
func sendTemplatedEmail(email, subject, preHeader, greeting, content, link, cta string) error {
	em := manager.GetInstance().EmailManager
...
func sendConfirmOldEmail(user models.User, activationKey uuid.UUID) error {
	...
	return sendTemplatedEmail(user.Email, subject, preHeader, greeting, content, link, cta)
}

func sendNewUserEmail(email string, activationKey uuid.UUID) error {
	...
	return sendTemplatedEmail(email, subject, preHeader, greeting, content, link, cta)
}

func sendResetPasswordEmail(user db.User, activationKey uuid.UUID) error {
...
func sendConfirmNewEmail(user *models.User, email string, activationKey uuid.UUID) error {
