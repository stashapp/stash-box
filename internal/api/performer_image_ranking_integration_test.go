...
func (s *performerRankingTestRunner) readGroupsForRanking() []models.ImageTypeGroup {
	s.t.Helper()
...
func (s *performerRankingTestRunner) restoreRankingOrder(groups []models.ImageTypeGroup) {
...
func (s *performerRankingTestRunner) testShippedDefaultOrder() {
	...
	// Shot type outranks Crop, so a perfectly cropped tattoo close-up cannot
	// take the primary slot from a portrait that is not itself a face crop.
	// This is the ordering doing real work.
	assert.Equal(s.t, imageC, s.orderedImages(performerID)[0])
	assert.NotEqual(s.t, imageE, s.orderedImages(performerID)[0])
}
...
func (s *performerRankingTestRunner) testAdminReorderChangesPrimary() {
	...
	before := s.readGroupsForRanking()
	defer s.restoreRankingOrder(before)
