...
| `images.format` | `webp` | The format used to store images. Can be set to one of the following: `png`, `jpeg`, `webp` or `avif`. Note: AVIF is not yet supported in every modern browser, see [Can I use AVIF](https://caniuse.com/avif) for more information. |
...
| `images.keep_original` | `true` | If true, the original and unaltered image will be kept in storage with its original size and format. |
