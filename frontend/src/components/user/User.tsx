...
const AddUserComponent: React.FC = () => {
    ...
    const handleDelete = (status:boolean):void => {
        if (status)
            deleteUser({ variables: { input: { id: user.id } } }).then(() => history.push('/admin'));
        setShowDelete(false);
    };
    ...
    return (
        <>
            { deleteModal }
            { isAdmin() && (
                <Button className="float-right" variant="danger" disabled={showDelete || deleting} onClick={toggleModal}>Delete User</Button>
            )}
            { canEdit() && (
            ...
            <div className="row">
                ...
                <textarea disabled className="col">{user.api_key}</textarea>
            </div>
        </>
    );
};
