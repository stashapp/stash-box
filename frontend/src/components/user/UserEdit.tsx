...
import { UpdateUserMutation, UpdateUserMutationVariables } from 'src/definitions/UpdateUserMutation';
...
const EditUserComponent: React.FC = () => {
    ...
    const [updateUser] = useMutation<UpdateUserMutation, UpdateUserMutationVariables>(UpdateUser);
    ...
    return (
        <div>
            ...
            <hr />
            <UserEditForm user={userData} username={user.name} error={queryError} callback={doUpdate} />
        </div>
    );
};
