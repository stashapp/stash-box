...
const diffArray = (name: string, original: (string|number)[]|null, updated: (string|number)[]|null) => {
  const arrA = original ?? [];
  const arrB = updated ?? [];
...
const diffBodyMod = (name: string, original: BodyModificationInput[]|null, updated: BodyModificationInput[]|null) => {
  const arrA = (original ?? []).map(b => `${b.location}${b.description ? ': ' : ''}${b.description || ''}`);
  const arrB = (updated ?? []).map(b => `${b.location}${b.description ? ': ' : ''}${b.description || ''}`);
...
const diffValue = (name: string, original: string|number|null, updated: string|number|null) => {
...
const DiffPerformer = (original: Performer, updated: PerformerFormData): ChangeRowProps[] => {
  const changes = [];

  changes.push(diffValue('Name', original.name, updated.name));
  changes.push(diffValue('Disambiguation', original.disambiguation, updated.disambiguation));
  ...
  changes.push(diffValue('Birthdate', original.birthdate?.date, updated.birthdate));
  ...
  changes.push(diffValue('Career Start', original.career_start_year, updated.career_start_year));
  changes.push(diffValue('Career End', original.career_end_year, updated.career_end_year));
  ...
  changes.push(diffArray('ImageIDs', original.images.map(i => i.id), updated.images));
