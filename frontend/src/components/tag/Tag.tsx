...
import { SortDirectionEnum, CriterionModifier } from 'src/definitions/globalTypes';
...
const TagComponent: React.FC = () => {
    ...
    const { data } = useQuery<Scenes, ScenesVariables>(ScenesQuery, {
        ...
        variables: {
            ...
            sceneFilter: { tags: { value: [tag && tag.findTag.id], modifier: CriterionModifier.INCLUDES } }
        }
    });
    ...
    return (
        <>
            <div className="row">
                <h3 className="col-4">Scenes for tag <em>{tag.findTag.description}</em></h3>
